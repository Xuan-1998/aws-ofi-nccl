 ...
 * single consumer). Each slot carries a sequence stamp that encodes whose
 * turn the slot is on:
 *
 *   - A producer reserves a slot by CAS-advancing enqueue_pos. It owns the
 *     slot whose stamp equals the position it reserved. It writes the payload,
 *     then publishes by storing stamp = pos + 1 (release).
 *   - The consumer owns the slot whose stamp equals dequeue_pos + 1. It reads
 *     the payload, then frees the slot by storing stamp = pos + CAPACITY
 *     (release), making it available to the producer one lap later.
 *
 * The stamp comparison is wrap-safe: producers compute (stamp - pos) as a
 * signed difference, so it stays correct across the uint32_t rollover of the
 * monotonic positions.
 *
 * Correctness highlights (full argument in the task writeup):
 *   - No torn reads: a slot's payload is written before its publish-store and
 *     read after the matching publish-load, paired release/acquire on stamp.
 ...
 *     stamp lap-offset (pos vs pos + CAPACITY) keeps a slow producer from
 *     colliding with the next lap's consumer because the stamp won't match
 *     until the consumer has freed the slot.
 *
 * CAPACITY must be a power of two so the slot index is a mask, not a modulo,
 * and so the lap arithmetic on the sequence stamp is exact. Unlike the SPSC
 * ring, every slot is usable: full/empty are told apart by the stamps, not by
 * keeping one slot empty, so the ring holds up to CAPACITY entries.
 *
...
class nccl_ofi_mpsc_ring {
	...
	/* Slot array. Sized to CAPACITY; each cell's sequence is seeded to its
	   index so the first lap of producers finds stamp == pos. */
	cell ring[CAPACITY];
	...
	alignas(64) std::atomic<uint32_t> dequeue_pos{0};
...
public:
	...
	bool pop(T &entry)
	{
		uint32_t pos = dequeue_pos.load(std::memory_order_relaxed);
