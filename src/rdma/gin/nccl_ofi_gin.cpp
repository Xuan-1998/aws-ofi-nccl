      ...
      ag_comm(s_comm_, r_comm_, rank_, nranks_)
{
	 ...
	 * fall back to running gdrcopy on the proxy thread in
	 * do_gin_signal_and_trace by leaving gdrcopy_thread_started = 0. */
	...
	if (pt_rc == 0) {
		...
		NCCL_OFI_WARN("Failed to spawn GIN gdrcopy worker thread: %d, "
...
{
	/* Stop and join the gdrcopy worker thread. The worker is busy-polling,
	 * so it will see gdrcopy_thread_stop on its next loop iteration. */
...
int nccl_ofi_rdma_gin_put_comm::iputSignal(uint64_t srcOff, nccl_ofi_gin_symm_mr_handle_t *srcMhandle, size_t size,
				  ...
				  bool aggregate, nccl_ofi_gin_req_t **request)
{
	...
	/* The single rail used by this op for both the write and the metadata
	   send. Determined after acquiring ep_lock (pinned or round-robin). */
	uint16_t rail_id;
	...
	/* Pick the rail. If a prior aggregated op pinned a rail (it was posted
	   with FI_MORE), reuse it so this op's doorbell flushes it. Otherwise
	   round-robin. We don't consult the scheduler (single-rail assumption). */
	...
	if (size > 0) {
		/* Post a single write-immediate request with user data on the
		   chosen rail (single-rail assumption; no scheduler). */
		...
		/* FI_MORE coalesces this write with the metadata send on the
		   same rail (put+signal). When aggregating, it also defers the
		   doorbell so the next iputSignal (pinned to this rail) flushes. */
	...
	if (has_signal) {
		...
		/* Unless aggregating, this send flushes the QP work queue on
		   the rail, issuing a doorbell that includes the coalesced
		   writedata WQE posted with FI_MORE above. When aggregating it
		   is itself posted with FI_MORE and flushed by the next op. */
...
void nccl_ofi_rdma_gin_put_comm::run_gdrcopy_worker_loop()
{
	...
	std::vector<gin_signal_work_entry> run;
	run.reserve(64);
	...
	while (!gdrcopy_thread_stop.load(std::memory_order_acquire)) {
		if (!gdrcopy_work_queue.pop(w)) {
			/* Empty queue: busy-poll. Hot signal-delivery path
			 * cannot afford the futex syscall round-trip. The
		 ...
		 * a new one. */
		if (!run.empty() &&
		    (w.metadata.signal_base_address !=
			run.front().metadata.signal_base_address ||
		     w.metadata.signal_offset !=
			run.front().metadata.signal_offset)) {
			apply_run();
		}
		run.push_back(w);
		...
		while (gdrcopy_work_queue.pop(w)) {
			if (w.metadata.signal_base_address ==
				run.front().metadata.signal_base_address &&
			    w.metadata.signal_offset ==
				run.front().metadata.signal_offset) {
				run.push_back(w);
