...
int nccl_ofi_rdma_gin_put_comm::retire_completed_peer_iput_ops(uint32_t peer_rank)
{
	...
	/* Walk in-order delivered ops, advancing rx_consumed as we go. The
	   ...
	   advancement until the worker has actually applied the gdrcopy and
	   cleared gdrcopy_in_flight, otherwise the sender could observe the
	   ACK before the local GPU sees the signal value. */
	while (true) {
		auto &rank_comm = this->rank_comms[peer_rank];
